...
export default function DestinationCard({ origin, city, country, destination, totalPrice, taxAmount, date, airline, duration, stops = 0, dealScore, dealClassification, valueScore, historicalPrice, destinationEmoji, bookingUrl, region, priceInsight, fare, trend, index = 0 }: DestinationCardProps) {
  ...
  return (
    <motion.article initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.28, delay: index * 0.04 }} className="group flex h-full flex-col overflow-hidden rounded-[1.6rem] border border-slate-200 bg-white shadow-[0_16px_42px_rgba(15,23,42,0.1)] transition hover:-translate-y-1 hover:shadow-[0_24px_52px_rgba(15,23,42,0.16)]">
      <div className="relative aspect-[16/10] overflow-hidden bg-slate-200">
        {!imageFailed ? <Image src={imageSet.landscape} alt={`${city} destination`} fill className="object-cover transition duration-500 group-hover:scale-105" sizes="(max-width: 768px) 100vw, (max-width: 1400px) 50vw, 33vw" loading="lazy" onError={() => setImageFailed(true)} /> : <div className="flex h-full items-center justify-center bg-gradient-to-br from-blue-700 to-indigo-900 text-7xl">{destinationEmoji}</div>}
      ...
      <div className="flex flex-1 flex-col gap-4 p-5 md:p-6">
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50/80 p-3">
          <div className="flex items-center gap-3">
            {!logoFailed ? <Image key={`${airline}-${logoIndex}`} src={logoSrc} alt={`${airlineBrand.name} logo`} width={40} height={40} className="h-10 w-10 rounded-xl border border-slate-200 bg-white p-1 object-contain" loading="lazy" onError={() => { if (logoIndex < airlineBrand.logoUrls.length - 1) return setLogoIndex((prev) => prev + 1); setLogoFailed(true); }} /> : <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-600">{airlineBrand.code}</div>}
        ...
        <div className="flex flex-wrap gap-2">{fare.amenities.slice(0, 4).map((amenity) => (<span key={amenity} className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-medium text-slate-700">{amenity}</span>))}</div>
        ...
        <div className="rounded-2xl border border-violet-100 bg-violet-50 p-3">
          ...
          <div className="grid grid-cols-4 gap-1 text-[11px] text-slate-600">{trend.map((t) => (<p key={t.label} className="text-center">{t.label}<br />{formatPrice(t.price, currency, rates).replace(`${currency} `, "")}</p>))}</div>
        </div>

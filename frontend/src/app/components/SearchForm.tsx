...
export default function SearchForm({ onSearch }: SearchFormProps) {
  ...
  const inputClassName =
    "h-12 w-full rounded-xl border border-slate-200 bg-white/95 px-4 text-sm font-medium text-slate-900 shadow-[0_2px_12px_rgba(15,23,42,0.05)] transition placeholder:text-slate-400 focus:border-violet-400 focus:outline-none focus:ring-4 focus:ring-violet-100";

  return (
    <form onSubmit={handleSubmit} className="rounded-[2rem] border border-white/45 bg-white/95 p-5 shadow-[0_26px_80px_rgba(15,23,42,0.24)] backdrop-blur-xl md:p-7">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-violet-700">Smart search</p>
          <p className="text-sm text-slate-600">Human-friendly destination input with structured airport matching.</p>
        </div>
        <span className="rounded-full bg-violet-50 px-3 py-1 text-xs font-semibold text-violet-700">Includes taxes + value scoring</span>
      </div>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <p className="text-xs text-slate-500">Tip: try region keywords like Europe, Asia, or Beach for inspiration.</p>
      </div>

      {error && <p className="mt-3 rounded-xl bg-rose-50 px-3 py-2 text-sm font-medium text-rose-700">{error}</p>}
    </form>
  );
}
